...
use Chattanooga\RobustMcp\LegacySessionSerializationMiddleware;
...
if ('/healthz' === $path) {
    ...
    $jsonResponse(200, ['status' => 'ok', 'server' => ROBUST_MCP_NAME, 'version' => ROBUST_MCP_VERSION, 'check' => 'health']);
}
...
if ('/readyz' === $path) {
    ...
    try {
        ...
        $jsonResponse(503, ['status' => 'not_ready', 'server' => ROBUST_MCP_NAME, 'version' => ROBUST_MCP_VERSION, 'check' => 'ready']);
    }
    $jsonResponse(200, ['status' => 'ok', 'server' => ROBUST_MCP_NAME, 'version' => ROBUST_MCP_VERSION, 'check' => 'ready']);
}
...
$configurationFailure = static function () use ($jsonResponse): never {
    $jsonResponse(500, ['error' => 'server_configuration_error', 'error_description' => 'The MCP server configuration is invalid.']);
};

try {
    $configuration = ServerConfiguration::fromEnvironment();
    $configuration->prepareForServing();
    $sessionLocks = new SessionLockCoordinator($configuration->sessionDirectory);
    $sessionStore = new LockAwareFileSessionStore($configuration->sessionDirectory, $configuration->sessionTtl, $sessionLocks);
...
if ($isPotentialOAuthMetadata && (null === $oauthRuntime || !in_array($path, $oauthRuntime->metadata->getMetadataPaths(), true))) {
    $jsonResponse(404, ['error' => 'not_found']);
}
...
$builder = Server::builder()
    ...
    ->setCachePolicy(CachePolicy::default(0, CacheScope::Private)->withMethod('tools/list', 300_000, CacheScope::Private))
...
$readOnlyAnnotations = new ToolAnnotations(readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false);
...
$tools->addTool(
    ...
    inputSchema: [
        ...
        'type' => 'object',
        '$defs' => ['tags' => ['type' => 'array', 'items' => ['type' => 'string', 'minLength' => 1, 'maxLength' => 32], 'maxItems' => 8, 'uniqueItems' => true]],
        'properties' => [
            'text' => ['type' => 'string', 'minLength' => 1, 'maxLength' => 256, 'x-mcp-header' => 'Text'],
            'mode' => ['type' => 'string', 'enum' => ['plain', 'counted']],
            'count' => ['type' => 'integer', 'minimum' => 1, 'maximum' => 1000],
            'tags' => ['$ref' => '#/$defs/tags'],
            'meta' => ['type' => 'object', 'properties' => ['trace' => ['type' => 'string', 'maxLength' => 64]], 'additionalProperties' => false],
        ],
        'required' => ['text', 'mode'],
        'allOf' => [['if' => ['properties' => ['mode' => ['const' => 'counted']], 'required' => ['mode']], 'then' => ['required' => ['count']]]],
        'additionalProperties' => false,
    ],
    outputSchema: [
        '$schema' => SchemaGuard::DRAFT_2020_12,
        'type' => 'object',
        '$defs' => ['tags' => ['type' => 'array', 'items' => ['type' => 'string'], 'maxItems' => 8]],
        'properties' => [
            'echo' => ['type' => 'string'],
    ...
    callback: static function (array $arguments, ClientGateway $gateway) use ($counterFile): array {
        ...
        $result = ['echo' => $arguments['text'], 'source' => 'robust-mcp', 'tags' => $arguments['tags'] ?? []];
...
$tools->addTool(
    ...
    inputSchema: ['$schema' => SchemaGuard::DRAFT_2020_12, 'type' => 'object', 'properties' => new stdClass(), 'additionalProperties' => false],
    outputSchema: [
        '$schema' => SchemaGuard::DRAFT_2020_12,
        'type' => 'object',
...
if (null !== $oauthRuntime) {
    $middleware[] = new ProtectedResourceMetadataMiddleware(metadata: $oauthRuntime->metadata, responseFactory: $factory, streamFactory: $factory);
    $middleware[] = new AuthorizationMiddleware(validator: $oauthRuntime->validator, resourceMetadata: $oauthRuntime->metadata, responseFactory: $factory);
} elseif ('manual-bearer' === $configuration->authMode) {
    try {
        $middleware[] = new ManualBearerAuthMiddleware(expectedSha256: (string) $configuration->bearerSha256, responseFactory: $factory, streamFactory: $factory);
    } catch (\InvalidArgumentException) {
        $configurationFailure();
    }
}

$middleware[] = new LegacySessionSerializationMiddleware(
    sessionDirectory: $configuration->sessionDirectory,
    locks: $sessionLocks,
    timeoutMs: $configuration->sessionLockTimeoutMs,
    responseFactory: $factory,
    streamFactory: $factory,
);
