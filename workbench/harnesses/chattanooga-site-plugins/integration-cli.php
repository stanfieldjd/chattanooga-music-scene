...
cms_site_plugins_assert( defined( 'CUA_VERSION' ) && '1.2.48' === CUA_VERSION, 'Chattanooga CMS Admin 1.2.48 did not load.' );
cms_site_plugins_assert( defined( 'CMS_MARKETPLACE_VERSION' ) && '0.1.1' === CMS_MARKETPLACE_VERSION, 'Marketplace 0.1.1 did not load.' );
cms_site_plugins_assert( defined( 'CMS_CORE_VERSION' ) && '0.2.4' === CMS_CORE_VERSION, 'Weekend Feature 0.2.4 did not load.' );
...
$feature_fixture = array(
	...
	(object) array( 'event_start_date' => '2026-09-11', 'event_start_time' => '20:00:00', 'event_name' => 'Friday Support' ),
	(object) array( 'event_start_date' => '2026-09-12', 'event_start_time' => '17:00:00', 'event_name' => 'Saturday Anchor' ),
	(object) array( 'event_start_date' => '2026-09-12', 'event_start_time' => '21:00:00', 'event_name' => 'Saturday Support' ),
	(object) array( 'event_start_date' => '2026-09-13', 'event_start_time' => '16:00:00', 'event_name' => 'Sunday Anchor' ),
	(object) array( 'event_start_date' => '2026-09-13', 'event_start_time' => '20:00:00', 'event_name' => 'Sunday Support' ),
);
$feature_picks = cms_site_plugins_private( $weekend, 'feature_events', array( $feature_fixture, $window ) );
cms_site_plugins_assert( 5 === count( $feature_picks ), 'Weekend Feature page must be limited to five curated highlights.' );
$feature_names = array_map( static function ( $event ) { return $event->event_name ?? ''; }, $feature_picks );
cms_site_plugins_assert( in_array( 'Friday Anchor', $feature_names, true ), 'Weekend Feature page omitted Friday representation.' );
cms_site_plugins_assert( in_array( 'Saturday Anchor', $feature_names, true ), 'Weekend Feature page omitted Saturday representation.' );
cms_site_plugins_assert( in_array( 'Sunday Anchor', $feature_names, true ), 'Weekend Feature page omitted Sunday representation.' );
...
echo "cms-site-plugins-integration: PASS cms_admin=1.2.48 marketplace=0.1.1 weekend_feature=0.2.4 wordpress_native_install=verified coexistence=verified marketplace_awp=verified marketplace_woocommerce=verified marketplace_search=verified marketplace_truncation=verified woocommerce_label=absent location_filter=preserved weekend_events_manager=verified weekend_schedule=verified cms_admin_health=verified database=verified admin_boundary=verified\n";
exit( 0 );
