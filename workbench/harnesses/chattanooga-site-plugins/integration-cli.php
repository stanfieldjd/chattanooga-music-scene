...
cms_site_plugins_assert( defined( 'CUA_VERSION' ) && 1 === preg_match( '/^\\d+\\.\\d+\\.\\d+$/', CUA_VERSION ), 'Chattanooga CMS Admin version did not load as a semantic version.' );
...
echo "cms-site-plugins-integration: PASS cms_admin=" . CUA_VERSION . " marketplace=0.1.1 weekend_feature=0.2.2 wordpress_native_install=verified coexistence=verified marketplace_awp=verified marketplace_woocommerce=verified marketplace_search=verified marketplace_truncation=verified woocommerce_label=absent location_filter=preserved weekend_events_manager=verified weekend_schedule=verified cms_admin_health=verified database=verified admin_boundary=verified\n";
exit( 0 );
