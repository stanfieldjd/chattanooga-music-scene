...
function cmsa_v2_rm_ability( $target ) {
	...
	if ( 1 !== count( $matches ) ) {
		...
		cmsa_v2_rm_fail(
			sprintf(
				'Expected one bridged Rank Math ability for %1$s; found %2$d. direct=%3$s init=%4$d abilities_init=%5$d class=%6$s file=%7$s rank_math_ability_hooks=%8$d rank_math_category_hooks=%9$d meta=%10$s input_schema=%11$s output_schema=%12$s expected_bridge=%13$s direct_bridge=%14$s registry_target=%15$s catalog_target=%16$s catalog_item=%17$s direct_method_item=%18$s execute_item=%19$s before_direct_item=%20$s',
				$target,
				count( $matches ),
