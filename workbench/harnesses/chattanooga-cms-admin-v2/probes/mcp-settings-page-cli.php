...
cmsa_mcp_settings_assert( false !== strpos( $rewrite_rules, '^\\.well-known/oauth-protected-resource/?
...
exit( 0 );
 ), 'Root protected-resource rewrite rule is missing.' );
...
cmsa_mcp_settings_assert( false !== strpos( $rewrite_rules, '^\\.well-known/oauth-authorization-server/?
$oauth_fallback_token = 'cmsa-oauth-fallback-' . wp_generate_password( 40, false, false );
$oauth_fallback_key = 'cua_oauth_access_' . hash_hmac( 'sha256', $oauth_fallback_token, wp_salt( 'auth' ) );
set_transient(
	$oauth_fallback_key,
...
exit( 0 );
 ), 'Authorization-server rewrite rule is missing.' );
