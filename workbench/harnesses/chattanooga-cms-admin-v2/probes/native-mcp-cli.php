...
cmsa_native_mcp_assert( in_array( '2026-07-28', $discover_data['result']['supportedVersions'] ?? array(), true ), 'Modern discovery did not advertise MCP 2026-07-28.' );
cmsa_native_mcp_assert( false === ( $discover_data['result']['capabilities']['tools']['listChanged'] ?? null ), 'Modern discovery returned the wrong tools capability.' );
...
cmsa_native_mcp_assert( 'chattanooga-cms-admin' === ( $discover_data['result']['_meta']['io.modelcontextprotocol/serverInfo']['name'] ?? '' ), 'Modern discovery did not identify the Chattanooga CMS Admin server.' );
...
$health = cmsa_native_mcp_modern( 'tools/call', array( 'name' => 'cmsa.get-health', 'arguments' => array() ), 103 );
...
cmsa_native_mcp_assert( get_bloginfo( 'version' ) === ( $health_data['result']['structuredContent']['wordpress_version'] ?? '' ), 'MCP get-health returned the wrong WordPress version.' );

$missing = cmsa_native_mcp_modern( 'tools/call', array( 'name' => 'cmsa.not-a-real-tool', 'arguments' => array() ), 104 );
...
$mismatch = cmsa_native_mcp_modern( 'server/discover', array(), 105, array( 'Mcp-Method' => 'tools/list' ) );
...
$origin_request->set_body( wp_json_encode( array( 'jsonrpc' => '2.0', 'id' => 106, 'method' => 'server/discover', 'params' => array( '_meta' => array( 'io.modelcontextprotocol/protocolVersion' => '2026-07-28' ) ) ) ) );
$origin = rest_do_request( $origin_request );
cmsa_native_mcp_assert( 403 === $origin->get_status(), 'Untrusted Origin was not rejected.' );

$legacy = cmsa_native_mcp_post( 'initialize', array( 'protocolVersion' => '2025-11-25', 'capabilities' => array(), 'clientInfo' => array( 'name' => 'cmsa-legacy-probe', 'version' => '1.0.0' ) ), array(), 107 );
...
cmsa_native_mcp_assert( 'chattanooga-cms-admin' === ( $legacy_data['result']['serverInfo']['name'] ?? '' ), 'Legacy initialize returned the wrong server identity.' );
...
wp_set_current_user( 1 );
echo "cmsa-native-mcp: PASS version=1.2.3 modern=2026-07-28 legacy=2025-11-25 route=verified oauth_challenge=verified admin_boundary=verified origin_guard=verified tools_list=deterministic read_call=verified private_bridges=hidden header_validation=verified\n";
exit( 0 );
