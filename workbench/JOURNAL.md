...
## 2026-09-09 — CMS Admin guarded media replacement integration

- Continued the media-lifecycle task from the verified 97-ability workbench position and kept production WordPress, `main`, `feature/chattanooga-cms-admin`, live Media Library records, miniOrange policy, and all unrelated site state outside the mutation set.
- Native WordPress 7.1 diagnostics established the replacement contract before admission: same-ID/same-path/same-MIME ordinary-image replacement preserves attachment URL, parent, featured-image relationships, and direct same-URL content references, but old image dimensions can leave obsolete derivatives that require explicit cleanup.
- A dedicated lifecycle state token was added for destructive file operations because execution proved the generic media token could miss same-size/same-mtime primary-byte changes, derivative-byte changes, attachment-metadata changes, and backup-size metadata changes. The lifecycle token binds exact attachment/backup metadata and hashes of the primary plus metadata-owned files while ordinary list-media remains lightweight.
- Guarded replacement source checkpoint `5b838c8b5ce9052a47e5e90a9a7c1088de204cd5` added `replace-media` as the sixth media ability and kept create/replace payload validation centralized at the existing 8 MiB, extension/type, payload-MIME, and no-remote-fetch boundary.
- Dedicated runtime `34395581366` passed the complete 98-ability registry, media object-scope permissions, existing media regressions, stale/incomplete/no-change/same-MIME guards, complex backup/companion-image fail-closed behavior, protected ID/path/URL/parent/MIME/reference invariants, obsolete derivative removal, and exact rollback after injected cleanup failure.
- PR #12 broad pre-integration gate passed Integrity `34395902699`, WooCommerce `34395902635`, Events Manager `34395902587`, Media Lifecycle `34395902642`, Content `34395902661`, Workbench `34395902639`, and Marketplace `34395902619`.
- PR #12 merged only into `workbench/mars` at `abc7b877bc4648434ac3021ad8ca8e9da2b52fc1`. Post-integration runs Content `34397966187`, Events Manager `34397966283`, WooCommerce Product `34397966192`, CMS Admin Workbench `34397966219`, and Mars Integrity `34397966263` all passed.
- The workbench candidate is now 98 abilities with six media abilities. Production remains independently verified at 82 exposed abilities; no deployment or live media/content/taxonomy/commerce mutation occurred.
- Permanent media deletion remains intentionally unadmitted. Native deletion clears featured-image relationships but leaves direct post-content URLs unchanged and potentially broken, and public deletion also requires independent owned-file absence verification. The containing media-lifecycle task therefore remains incomplete pending a separately execution-verified delete reference-protection contract.
- STATUS, TASK_QUEUE, the media-lifecycle task record, and machine-readable state were reconciled to the 98-ability position in the same workbench records transaction. The next gate is workbench integrity validation of the reconciled records, then source-only permanent-delete contract research from the verified current workbench checkpoint.
