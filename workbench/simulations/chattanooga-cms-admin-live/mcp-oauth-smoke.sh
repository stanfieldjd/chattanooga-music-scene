...
{"client_name":"CMSA Native OAuth Smoke","redirect_uris":["${CALLBACK_URI}"],"grant_types":["authorization_code","refresh_token"],"response_types":["code"],"token_endpoint_auth_method":"none","application_type":"native"}
JSON
...
client_id="$(docker compose run --rm -T cli php -r '$d=json_decode(stream_get_contents(STDIN),true); if (empty($d["client_id"]) || ($d["token_endpoint_auth_method"]??"")!=="none" || ($d["application_type"]??"")!=="native") exit(1); echo $d["client_id"];' <"$tmp_dir/register-response.json")"
test -n "$client_id"
...
grep -Fq "Client identifier: <code>${client_id}</code>" "$tmp_dir/authorize.html"
...
echo 'cmsa-native-mcp-oauth: PASS protected_resource=verified metadata_path=resource-derived authorization_server=verified dcr=native admin_consent=verified client_identity=visible redirect_hostname=visible pkce=verified bearer_mcp=verified refresh_rotation=verified accept=dual envelope=verified third_party_mcp=absent'
