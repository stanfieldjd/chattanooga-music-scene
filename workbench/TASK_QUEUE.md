...
| --- | --- | --- | --- |
| 1 | Chattanooga CMS Admin | CONTENT_LAYER_ACTIVE | Continue bounded Layer B in the workbench: status transitions, taxonomy relationships, then media relationships. Keep production installation/MCP discovery separate until an explicit deployment rollback transaction is authorized. |
...
- `CONTENT_LAYER_ACTIVE` is workbench-only engineering; it does not authorize installation or live content mutation.
