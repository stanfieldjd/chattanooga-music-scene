...
| --- | --- | --- | --- |
| 1 | Chattanooga CMS Admin | LIVE_MCP_RUNTIME_VERIFIED_WEEKEND_SOURCE_INTEGRATED | Production plugin 0.1.0 remains at 82 exposed abilities with live health and exact event-taxonomy reads verified. The workbench candidate is now 91 abilities after execution-verified integration of five media abilities and four Weekend Feature abilities. Preserve the production/source distinction; any production promotion remains a separate A3 action. Continue source-only functionality engineering from the verified workbench position. |
...
- `LIVE_MCP_RUNTIME_VERIFIED_WEEKEND_SOURCE_INTEGRATED` distinguishes two verified states: the installed production runtime currently exposes 82 abilities, while the source/workbench candidate contains 91 after the five-ability media slice and four-ability Weekend Feature slice passed integration CI. Workbench integration does not authorize or prove production deployment.
- The Weekend Feature source adapter is execution-verified for bounded status, exact settings replacement, guarded current-weekend draft generation, and guarded immediate publication. The authoritative Weekend Feature dependency is workbench version 0.2.2 after a direct source repair for first-save schedule synchronization. Production promotion of the source dependency or the four new abilities requires separate A3 authorization; live publication additionally requires its own external-effect authorization.
- The media source slice is execution-verified for bounded list/get, validated 8 MiB base64 creation, exact-state metadata mutation, and exact-state featured-image relationship management. Production promotion requires separate A3 authorization followed by fresh live MCP discovery and read validation.
...
- Media and Weekend Feature administration are no longer unstarted capability targets. Media replacement/permanent deletion, recurrence, bookings, tickets, payments, account security, widgets/templates, location deletion, generic option mutation, and additional site-specific adapters remain non-automatic targets requiring their own evidence, design, and authorization.
