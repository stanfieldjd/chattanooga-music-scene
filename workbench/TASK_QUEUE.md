...
| 4 | Venue/location data quality | LIVE_DEFECT_INVENTORY_VERIFIED_RESEARCH_AND_WRITE_PENDING | Exact live reads confirm three impossible/default `47.4,1.6` coordinate records, Ross’s Landing region-field contamination, two missing-postcode records, and a broader zero-coordinate class. Establish authoritative replacement values before any exact-state write; preserve intentional logical locations such as Multiple Chattanooga Venues. |
...
- `LIVE_DEFECT_INVENTORY_VERIFIED_RESEARCH_AND_WRITE_PENDING` means production location defects have been execution-verified but replacement values are not yet established and no live `update-location` authorization exists. Research and mutation remain separate gates.
