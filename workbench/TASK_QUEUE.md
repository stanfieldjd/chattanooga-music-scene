...
| --- | --- | --- | --- |
| 1 | Chattanooga CMS Admin | LIVE_MCP_RUNTIME_VERIFIED_MEDIA_SOURCE_INTEGRATED | Production plugin 0.1.0 remains at 82 exposed abilities with live health and exact event-taxonomy reads verified. The workbench candidate is now 87 abilities after execution-verified integration of five bounded media abilities. Preserve the production/source distinction; any media deployment is a separate A3 action. |
...
- `LIVE_MCP_RUNTIME_VERIFIED_MEDIA_SOURCE_INTEGRATED` distinguishes two verified states: the installed production runtime currently exposes 82 abilities, while the source/workbench candidate contains 87 after the five-ability media slice passed integration CI. Workbench integration does not authorize or prove production deployment.
...
- Media is no longer an unstarted capability target: its current bounded source slice is integrated and verified. Media replacement/permanent deletion, recurrence, bookings, tickets, payments, account security, widgets/templates, location deletion, and generic option mutation remain non-automatic targets requiring their own evidence, design, and authorization.
